...
# suppl 1. Spider traits -----------------------------------------------

writexl::write_xlsx(list(arachnida = 
readxl::read_excel("clean_data9.xlsx", sheet = "Arachnida.Traits") %>% 
  transmute(taxa = substr(taxa, 1, nchar(taxa)-2), humidity, habitats, lifeform, 
            storey__, sex, size = paste0(round(size_exact, 2)," (", size____, ")")) %>% 
  pivot_wider(names_from = sex, values_from = size, values_fill = "-") %>%
  transmute(taxa, size = paste0("mmm: ", m, "; fff: ", f), humidity, 
            habitats,lifeform, storey = storey__)
,
carabidae = readxl::read_excel("clean_data9.xlsx", sheet = "Carabidae.Traits") %>% 
  transmute(taxa, fenol, foraging, storey__, habitats, humidity, dispersl,
            size = paste0(size_exact, " (", size____, ")"), 
            N = paste0("n = ", `Измерено >`, " mmm + ", `Измерено +`, " fff"))
), 
"supplements1-2.xlsx")



# suppl 2. what is this name? ---------------------------------------------

aratraits %>% 
  group_by(year, zone) %>% 
  summarise( sd = sd(num), num = sum(num), .groups = "drop") %>% 
  mutate()
