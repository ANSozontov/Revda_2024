...
library(foreach)
library(tidyverse)
carlong <- readxl::read_excel("Data/clean_data8.xlsx", sheet = "Carabidae.Lines2") %>% 
  pivot_longer(names_to = "taxa", values_to = "num", -c("year", "zone", "line")) %>% 
  group_by(year, zone, line) %>% 
  mutate(part = num/sum(num)) %>% 
  ungroup()
cartraits <- readxl::read_excel("Data/clean_data8.xlsx", sheet = "Carabidae.Traits") %>% 
  select(taxa, fenol:size____) %>% 
  right_join(., carlong, by = "taxa") %>% 
  mutate(type = paste0(zone, "_", year))
aralong <- readxl::read_excel("Data/clean_data8.xlsx", sheet = "Arachnida.Lines2") %>% 
  pivot_longer(names_to = "taxa", values_to = "num", -c("year", "zone", "line")) %>% 
  group_by(year, zone, line) %>% 
  mutate(part = num/sum(num)) %>% 
  ungroup()
aratraits <- readxl::read_excel("Data/clean_data8.xlsx", sheet = "Arachnida.Traits") %>% 
  select(taxa,  size____:storey__) %>% 
  right_join(., aralong, by = "taxa") %>% 
  mutate(type = paste0(zone, "_", year))

# 2. Rarefication ---------------------------------------------------------
rar.car <- readxl::read_excel("Data/clean_data8.xlsx", sheet = "Carabidae.Lines2") %>% 
...
rar.ara <- readxl::read_excel("Data/clean_data8.xlsx", 
...
agg <- function(df1, gr, tax) {
  ...
  df1 %>% 
    ...
    pivot_wider(names_from = tp, values_from = num) %>% 
    return(.)
}
res <- expand_grid(year = c(2005,2018), 
                   zone = c("fon", "imp"), 
                   line = 1:7)
...
effs <- expand_grid(vr = colnames(res)[4:ncol(res)], year = c(2005, 2018)) 
effs <- cbind(effs, 
  foreach(i = 1:nrow(effs), .combine = rbind) %do% {
  SingleCaseES::LRRi( # effs[i, 3:7] <-  
    res[res$zone == "fon" & res$year == effs$year[i], effs$vr[i]],
    res[res$zone == "imp" & res$year == effs$year[i], effs$vr[i]])
  }
)
...
ara.eff <- effs %>% 
  ...
  mutate(year = as.character(year),
         tx = substr(vr, 1, 1), 
         vr = substr(vr, 3, nchar(vr)), 
         vr2= paste0(vr, "_", year), 
         nn = sort(c(seq(1, 44, by = 3), seq(2, 44, by = 3))), 
         vr2 = case_when(year == "2018" ~ vr, TRUE ~ "NA")) 
ara.eff <- ara.eff %>% 
  rbind(cbind(matrix(ncol = 9, nrow = 14, 
                     dimnames = list(NULL, colnames(ara.eff)[1:9])), 
              nn = seq(3, 44, by = 3))) %>% 
  arrange(nn)
ara.eff %>% 
  ggplot(aes(color = year, x = nn, 
    ...
    geom_vline(xintercept = seq(3, 68, by = 3), linetype = "dashed") +
    geom_pointrange() + 
    coord_flip() + 
    theme(legend.position = "bottom") +
    labs(y = "Effect Size", title = "Arachnida") +
    scale_x_discrete(limits = ara.eff$vr2) 
...
car.eff <- effs %>% 
  ...
  mutate(year = as.character(year),
         ...
         vr = substr(vr, 3, nchar(vr)), 
         vr2= paste0(vr, "_", year), 
         nn = sort(c(seq(1, 50, by = 3), seq(2, 50, by = 3)))) 
         # vr2 = case_when(year == "2018" ~ vr, TRUE ~ "NA")) 
car.eff <- car.eff %>% 
  rbind(cbind(matrix(ncol = 8, nrow = 16, 
                     dimnames = list(NULL, colnames(car.eff)[1:8])),
              vr2 = 8,
              nn = seq(3, 50, by = 3))) %>% 
   distinct() %>% 
  arrange(nn) %>% 
  mutate(vr2 = case_when(substr(vr2, nchar(vr2), nchar(vr2)) == "5" ~ 
                           substr(vr2, 1, nchar(vr2)-5), 
                         TRUE ~ "NA"))
car.eff %>% 
  ggplot(aes(color = year, x = nn, 
             y = Est, ymin = CI_lower, ymax = CI_upper)) + 
  geom_hline(yintercept = 0) +
  geom_vline(xintercept = seq(3, 50, by = 3), linetype = "dashed") +
  geom_pointrange() + 
  coord_flip() + 
  theme(legend.position = "bottom") +
  labs(y = "Effect Size", title = "Carabidae") +
  scale_x_discrete(limits = car.eff$vr2) 
